...
def dbconnection():
    connection = pymysql.connect(host='localhost', database='traffic_system', user='root', password='H_b_1726')
    return connection
...
@app.route('/search', methods=['GET', 'POST'])
def search_license_plate():
    number_plate_text = request.form.get('number_plate_text')
    connection = dbconnection()
    vehicle_data = []
    error = None
    if request.method == 'POST' and number_plate_text:
...
from atcc import *
from anpr_video import process_anpr_video
from ultralytics import YOLO
...
def start_processing():
    ...
    if process_type.lower() == 'atcc':
        ...
        for file in input_files:
            result = process_anpr_video(file)
            processed_results.append(result)
        return render_template('live_monitoring.html',
                               video_paths=[r["video_path"] for r in processed_results],
                               frame_paths=[r["frames"] for r in processed_results],
                               process="ANPR")
    ...
    try:
        ...
        def process_single_video(video_path):
            try:
                ...
                print(f"Error processing {video_path}: {e}")
        max_threads = min(len(input_files), 6)
        with ThreadPoolExecutor(max_threads) as executor:
            executor.map(process_single_video, input_files)
        detector.process_video_with_gui(input_files)
        while True:
